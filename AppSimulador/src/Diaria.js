...
export default class Diaria extends Component{
    ...
    static navigationOptions = {
        title : 'Simular valor diaria (comum)'
	}
	...
	diaria(){
		...
		fetch('https://simuladordiarias.herokuapp.com/diaria.php', {
			method: 'POST',	
			headers: {
				...
				<View style={styles.container}>
					<View style={styles.main}>			
						...
						<View style={[styles.BoxResponse, (this.state.msg != '')? {backgroundColor: '#1E90FF'} :{display: 'none'} ]}>
							...
							<Text style={styles.TextResponse}>{(this.state.horas_consedidas != '')? 'Horas Consedidas: ' + this.state.horas_consedidas: ''}</Text>
